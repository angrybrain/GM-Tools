...
export class GameMechanicsService {
  ...
  private betterValue(appropriateValue: number, firstValue: number, secondValue: number) {
  ...
  private worstValue(appropriateValue: number, firstValue: number, secondValue: number) {
  ...
  private isCriticalRoll(value: number) {
  ...
  private panicEffectRoll(value: number) {
    let effect: string;
    if (value >= 0 && value <= 3) {
  ...
  private saveCheck(saveType: string, saveValue: number, advantage: boolean = false, disadvantage: boolean = false) {
  ...
  private damageCheck(health: number, dmg: number, isCriticalRoll: boolean = false) {
  ...
  private armorCheck(armor: number, dmg: number, isCriticalRoll: boolean = false) {
    if (isCriticalRoll) {
      armor = armor / 2;
    }
    dmg -= armor;
  ...
  private statCheck(statType: string, statValue: number, advantage: boolean = false, disadvantage: boolean = false) {
    let passed: boolean = false;
    let critical: boolean = false;
    let firstTry: number;
    let secondTry: number;
  ...
  private aimTo(target: Number = 10) {
    if (target === 10) {
      return 'Head';
  ...
  public hitTarget(target: number, critical: boolean = false) {
    let damage = +prompt(`Roll for Damage`);
    let bodyPart: string = this.aimTo(target)
